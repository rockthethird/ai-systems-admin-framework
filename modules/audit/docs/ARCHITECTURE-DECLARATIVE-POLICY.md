...
committed. Human review displays exact generated bytes and concise provenance
for copied files, then binds approval to the complete bundle digest, including
every file byte and its installation metadata.
...
`policy.py review` requires an interactive terminal. Generated artifacts are
displayed exactly. Copied artifacts display their repository source,
destination, installation metadata, hash, and confirmed byte equality instead
of repeating reviewed source code. The canonical artifact index binds that
provenance and every artifact hash into the complete bundle digest. Approval is
